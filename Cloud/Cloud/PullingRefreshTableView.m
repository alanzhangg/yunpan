...
- (id)initWithFrame:(CGRect)frame atTop:(BOOL)top {
    ...
    if (self) {
        ...
        _stateLabel.text = NSLocalizedString(@"下拉同步", @"");
...
- (void)setState:(PRState)state animated:(BOOL)animated{
    ...
    if (_state != state) {
//        NSLog(@"_state_______%d___state______%d",_state,state);
        ...
        if (_state == kPRStateLoading) {    //Loading
            ...
            _loading = YES;
            if (self.isAtTop) {
                _stateLabel.text = NSLocalizedString(@"正在同步", @"");
            } else {
            ...
            if (!self.isAtTop) {    //footer
                _arrow.hidden = YES;
                _stateLabel.text = NSLocalizedString(@"没有更多内容了", @"");
            }
        }
    }
}
...
@synthesize hiddenAll= _hiddenAll;
@synthesize isFooterInAction=_isFooterInAction;
...
- (void)setHiddenAll:(BOOL)hiddenAll{
    ...
    _footerView.hidden = _hiddenAll;
    _headerView.hidden=_hiddenAll;
}
...
- (void)tableViewDidScroll:(UIScrollView *)scrollView {
    ...
    float yMargin = offset.y +  (size.height<contentSize.height ? (size.height - contentSize.height):0);
...
//        NSLog(@"___ENDDrag");
        ...
        [UIView animateWithDuration:kPRAnimationDuration animations:^{
            ...
            self.contentOffset = CGPointMake(0, -kPROffsetY);
        }];
...
- (void)flashMessage:(NSString *)msg{
    ...
    CGRect rect =CGRectMake(0, self.frame.origin.y, self.bounds.size.width, 20);
    
    //self.contentSize.height >self.bounds.size.height ?CGRectMake(0,  self.contentSize.height-self.bounds.size.height- 20, self.bounds.size.width, 20):CGRectMake(0, - 20, self.bounds.size.width, 20);
    ...
    if (_msgLabel == nil) {
        ...
        _msgLabel.backgroundColor = [UIColor colorWithRed:36.0/255.0 green:178.0/255.0 blue:251.0/255.0 alpha:1.0];
        _msgLabel.textAlignment = NSTextAlignmentCenter;
        
        if (self.superview) {
            [self.superview insertSubview:_msgLabel aboveSubview:self];
        }
        
    }
    _msgLabel.text = msg;
    _msgLabel.alpha=0.0f;
